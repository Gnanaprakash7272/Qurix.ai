...
const WS_BASE = window.location.hostname === 'localhost' ? 'ws://localhost:8000' : (window.location.protocol === 'https:' ? 'wss:' : 'ws:') + '//' + window.location.host;
...
function App() {
  ...
  const connectWebSocket = () => {
    const ws = new WebSocket(`${WS_BASE}/api/agent/ws`);
  ...
  const fetchVaultData = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/data/scraped`);
    ...
    return (
      <div className="auth-container">
        <div className="auth-box new-login-box">
          ...
          <div className="social-buttons" style={{ display: 'flex', gap: '15px', width: '100%' }}>
            <a href={`${API_BASE}/api/auth/login/github`} className="social-btn" style={{ textDecoration: 'none' }}>
            ...
            <a href={`${API_BASE}/api/auth/login/google`} className="social-btn" style={{ textDecoration: 'none' }}>
                          ...
                          {fileMatches.length > 0 && (
                            <div className="download-buttons-container" style={{ marginTop: '15px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                              {fileMatches.map((filePath, i) => (
                                <a 
                                  key={i} 
                                  href={API_BASE ? `${API_BASE}/${filePath}` : `/${filePath}`} 
                                  download
                                  target="_blank"
                                  rel="noreferrer"
                                  className="primary-action-btn"
